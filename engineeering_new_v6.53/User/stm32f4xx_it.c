...
/*  file (startup_stm32f429_439xx.s).  可以添加外围设备的中断                       */
/******************************************************************************/
...
void CAN1_RX0_IRQHandler(void)	//解析传回数据*****需增加
{
	...
	switch(RxMessage.StdId)
	{
		...
		break;
		case(CAN_3508Motor7_ID):
			record_motor_callback(&motor7, angle, speed, current);		
		break;
...
//中断服务函数			    
void CAN2_RX0_IRQHandler(void)	//解析传回数据*****需增加
{
  	CanRxMsg RxMessage;
	...
	switch(RxMessage.StdId)
	{
		case(CAN_2006Motor10_ID):
			record_motor_callback(&motor10, angle, speed, current);
		break;
		case(CAN_2006Motor11_ID):
			record_motor_callback(&motor11, angle, speed, current);
		break;
		case(CAN_3508Motor12_ID):
			record_motor_callback(&motor12, angle, speed, current);
		break;
		case(CAN_3508Motor13_ID):
			record_motor_callback(&motor3, angle, speed, current);
		break;
		case(CAN_2006Motor9_ID):
			record_motor_callback(&motor_can2_1, angle, speed, current);
		break;
		case(CAN_3508Motor6_ID):
	  	record_motor_callback(&motor6, angle, speed, current);
		break;
